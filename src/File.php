...
use phpfileuploader\movehandler\LocalMoveHandler;
use SplFileObject;

class File extends SplFileObject
{
    private static $file_move_handler = LocalMoveHandler::class;
    /**
     * 错误信息
     * @var string
     */
    ...
    /**
     * @param $name function name or class name
     */
    public static function setFileMoveHandler($name){
        self::$file_move_handler = $name;
    }
...
//        }
        ...
        if ($this->isTest) {
            ...
            if(!self::$file_move_handler){
                $this->error = 'please set file_move_handler by call File::setFileMoveHandler';
                return false;
            }
            $this->error = 'upload write error';
            if(class_exists(self::$file_move_handler)){
                if(!(self::$file_move_handler)::doMove(
